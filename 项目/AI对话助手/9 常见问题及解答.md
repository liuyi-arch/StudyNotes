...
**项目简介**：基于 Google Gemini API 的智能对话平台，支持文本和**语音输入**，具备**多轮对话、历史记录管理**、**打字动画**等功能，提供类 ChatGPT 的自然语言交互体验。
...
1. **你是如何与 Google Gemini API 对接的？使用的是什么通信方式？**✅补充

   1.1 follow-up：请求是否支持流式响应？你是怎么处理响应数据的？

   - **将密钥传入GoogleGenerative API，拿到返回的生成模型，然后通过sendMessage(prompt)向生成模型发送用户输入提示词并接受返回结果；**
   ...
   - **当前没有实现API级别的流式响应，而是通过await chat.sendMessage(prompt)等待API返回完整结果，然后通过定时器实现打字动画效果。也就是setTimeout设置一个等待时间比如50ms*index，index为每个字符索引；**
...
2. **你是如何管理多轮对话上下文的？上下文信息是怎么维护的？**✅补充

   2.1 follow-up：有没有做过对上下文长度或轮数的限制？为什么？

   - 使用数组prevPrompt、字符串recentPrompt分别保存用户输入的历史问题、最近一次会话的输入；使用setPrevprompt将新的prompt放置prevPrompt队尾，更新recentPrompt为Prompt；
...
3. **Web Speech API 是如何实现语音转文字的？兼容性如何？**✅
   ...
   - 通过`window.webkitSpeechRecognition()`创建recognition识别器实例，并为该recognition实例创建了onstart、onresult、onend事件处理函数；
   - onstart事件处理函数用于开启话筒状态，当用户点击话筒按钮触发；
   - onresult事件处理函数用来实时监听识别结果，并将语音识别的文本放在输入框、作为Prompt发送给模型；
   - onsent事件处理函数用来关闭话筒按钮状态，当用户停止说话或手动停止识别时触发；
   - 带webkit前缀的SpeechRecognition只有Chromium 系可用，即 Chrome、Edge；而Safari、Firefox 只能使用不带前缀SpeechRecognition，它不支持语音识别；
...
6. **你如何处理聊天记录的持久化？用的是哪种本地存储方案？是否考虑了数据量大的场景？**✅
   ...
   - localStorage和sessionStorage：前者永久有效，后者只在当前会话有效；

     ```js
     localStorage.setItem("username", "Alice"); // 保存数据
     const user = localStorage.getItem("username"); // 读取数据
     localStorage.removeItem("username"); // 删除某个键
     localStorage.clear(); // 清空所有数据
     // sessionStorage同样具备上述方法
     ```
...
10. **是否有统一封装 API 请求模块？请求失败如何处理？有没有做重试机制？**✅
    ...
    - runChat 函数可以看作是一个基础的 API 请求模块封装。它将模型初始化、配置和发送消息的逻辑聚合在一起。
      - 可以在 onSent函数中**使用 try...catch来包裹 await runChat(...)**；如果 API 请求因为**网络问题、API 密钥无效**等原因，在 catch块中，可以抛出对应错误状态；
      - 在 **catch块中，可以检查错误的类型（比如检查状态码网络超时504）**，如果符合重试条件，则可以等待一小段时间后再次调用；
